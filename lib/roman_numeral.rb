class RomanNumeral
  ...
  def validates?(roman_numeral)
    valid_numeral_pattern = /^M{0,3}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})$/
    roman_numeral =~ valid_numeral_pattern ? true : raise(StandardError, 'Roman numeral is invalid')
  end
